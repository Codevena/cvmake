...
Expected: `feat/cvmake-mvp` branch, only `?? data/cvs/photos/markus.webp` (untracked) and `M apps/web/next-env.d.ts` (auto-generated). No staged changes.
...
Note current structure — fixtures use Markus's data as input.

- [ ] **Step 7.2: Replace `valid.de.yaml` with mini-Lena**

Use sed for safe field-only replacement:
```bash
sed -i '' 's/Markus Wiesecke/Lena Bauer/g' packages/core/test/fixtures/valid.de.yaml
sed -i '' 's/Markus/Lena/g' packages/core/test/fixtures/valid.de.yaml
sed -i '' 's/Wiesecke/Bauer/g' packages/core/test/fixtures/valid.de.yaml
sed -i '' 's|markus.wiesecke@gmail.com|lena.bauer@example.com|g' packages/core/test/fixtures/valid.de.yaml
sed -i '' 's|photos/markus|photos/example-lena|g' packages/core/test/fixtures/valid.de.yaml
sed -i '' 's|+49 175 9025586|+49 30 0000000|g' packages/core/test/fixtures/valid.de.yaml
sed -i '' 's|Wuppertal, DE|Berlin, DE|g' packages/core/test/fixtures/valid.de.yaml
```

Then read the file and verify no remaining personal markers:
```bash
grep -i "markus\|wiesecke\|wuppertal\|9025586" packages/core/test/fixtures/valid.de.yaml
```
Expected: no matches.
...
```bash
sed -i '' 's/Markus Wiesecke/Lena Bauer/g' packages/core/test/fixtures/invalid-missing.de.yaml
sed -i '' 's/Markus/Lena/g' packages/core/test/fixtures/invalid-missing.de.yaml
sed -i '' 's/Wiesecke/Bauer/g' packages/core/test/fixtures/invalid-missing.de.yaml
sed -i '' 's|markus.wiesecke@gmail.com|lena.bauer@example.com|g' packages/core/test/fixtures/invalid-missing.de.yaml
sed -i '' 's|photos/markus|photos/example-lena|g' packages/core/test/fixtures/invalid-missing.de.yaml
sed -i '' 's|+49 175 9025586|+49 30 0000000|g' packages/core/test/fixtures/invalid-missing.de.yaml
sed -i '' 's|Wuppertal, DE|Berlin, DE|g' packages/core/test/fixtures/invalid-missing.de.yaml
```
...
```bash
sed -i '' 's/Markus Wiesecke/Lena Bauer/g' packages/schema/test/fixtures.ts
sed -i '' 's/Markus/Lena/g' packages/schema/test/fixtures.ts
sed -i '' 's/Wiesecke/Bauer/g' packages/schema/test/fixtures.ts
sed -i '' 's|markus.wiesecke@gmail.com|lena.bauer@example.com|g' packages/schema/test/fixtures.ts
sed -i '' 's|photos/markus|photos/example-lena|g' packages/schema/test/fixtures.ts
```
...
Expected: all pass. If any test asserts on `'Markus'` or `'Wiesecke'` literal, that's Group B (next task) — flag it.
...
for f in \
  ...
  apps/web/components/sections/PersonalSection.test.tsx; do
    sed -i '' 's/Markus Wiesecke/Lena Bauer/g' "$f"
    sed -i '' 's/Markus/Lena/g' "$f"
    sed -i '' 's/Wiesecke/Bauer/g' "$f"
    sed -i '' 's|markus.wiesecke@gmail.com|lena.bauer@example.com|g' "$f"
    sed -i '' 's|photos/markus|photos/example-lena|g' "$f"
done
```
...
Expected: all 178 tests pass. If any fails because it asserts on a Markus-specific string that wasn't covered, fix the assertion to match Lena.
...
- [ ] **Step 9.3: Verify no Markus markers in regenerated snapshots**

Run:
```bash
grep -i "markus\|wiesecke" packages/templates/test/__snapshots__/*.snap
```
...
Run:
```bash
grep -n "Markus" apps/web/app/dev-ui/page.tsx
```

Expected: line shows `placeholder="Markus"`.

- [ ] **Step 10.2: Replace with Alex**

Use Edit tool with old_string `placeholder="Markus"` and new_string `placeholder="Alex"`.

- [ ] **Step 10.3: Verify**

Run:
```bash
grep -i "markus" apps/web/app/dev-ui/page.tsx
```
...
**NOTE:** Do NOT modify `docs/superpowers/specs/2026-05-10-personal-data-strategy-design.md` — it's the spec for THIS migration and references "Markus" in the context-paragraph deliberately. Same for THIS plan file.
...
for f in \
  ...
  docs/template-review-2026-04-25.md; do
    sed -i '' 's/Markus Wiesecke/Alex Schmidt/g' "$f"
    sed -i '' 's/Markus/Alex/g' "$f"
    sed -i '' 's/Wiesecke/Schmidt/g' "$f"
    sed -i '' 's|markus.wiesecke@gmail.com|maintainer@example.com|g' "$f"
    sed -i '' 's|+49 175 9025586|+49 30 0000000|g' "$f"
    sed -i '' 's|Wuppertal|Berlin|g' "$f"
done
```
...
git check-ignore -v data/cvs/cv.de.yaml data/cvs/example.de.yaml \
  data/cvs/photos/markus.jpg data/cvs/photos/example-lena.webp \
...
- `data/cvs/photos/markus.jpg` → ignored
...
**Files:**
- Untrack (keep in working tree): `data/cvs/cv.de.yaml`, `cv.en.yaml`, `photos/markus.jpg`, `photos/markus.webp`
...
Run:
```bash
ls data/cvs/cv.de.yaml data/cvs/cv.en.yaml data/cvs/photos/markus.jpg
ls NEXT_SESSION.md
ls .review-logs/ | head -5
```

Expected: all files present (markus.webp may or may not exist — currently untracked anyway).
...
git rm --cached data/cvs/photos/markus.jpg
...
Run:
```bash
git ls-files data/cvs/cv.de.yaml data/cvs/cv.en.yaml data/cvs/photos/markus.jpg NEXT_SESSION.md
...
Expected: no output. The 2 excluded files reference Markus deliberately as the migration's context.
...
- [ ] **Step 14.4: Wuppertal-leak grep (location)**

Run:
```bash
git grep -i "wuppertal" -- ':!pnpm-lock.yaml'
```
...
git ls-files data/cvs/photos/markus.jpg | grep -q . && echo "FAIL: still tracked" || echo OK
...
If any of these fail, diagnose and fix before proceeding. Most likely failure: a test still asserts on Markus-specific content not caught by sed.
...
git commit -m "docs: scrub personal-data references — Markus → Alex pseudonym
...
cv.*.yaml + photos/markus.* are now local-only. Forkers' personal
...
- [x] Snapshots regenerated and contain no Markus references
...
If e2e+visual fails on baselines: visual regression is expected because the rendered content is now Lena/Adam instead of Markus. Solution:
...
Expected: commit messages that previously had "Markus" content now show "Alex"; old YAML reference (if any) shows scrubbed content.
...
```bash
git filter-repo --analyze
grep -i "markus\|wiesecke" .git/filter-repo/analysis/*.txt
```
...
- `data/cvs/` listing shows `example.de.yaml`, `example.en.yaml`, `photos/`, `.gitkeep` — NO `cv.de.yaml`/`cv.en.yaml`/`markus.*`
...
**Files:** Markus's local working tree at `/Users/markus/Developer/cvMake`
...
cd /Users/markus/Developer/cvMake
cp data/cvs/cv.de.yaml ~/cv-backup-2026-05-10/
cp data/cvs/cv.en.yaml ~/cv-backup-2026-05-10/
cp data/cvs/photos/markus.* ~/cv-backup-2026-05-10/ 2>/dev/null
ls ~/cv-backup-2026-05-10/
```

Expected: backup directory contains cv.de.yaml, cv.en.yaml, markus.jpg (and possibly markus.webp).

- [ ] **Step 30.2: Move existing clone aside (don't delete yet)**

Run:
```bash
cd /Users/markus/Developer
...
cp ~/cv-backup-2026-05-10/markus.* data/cvs/photos/ 2>/dev/null
git status
```
...
Run:
```bash
git check-ignore data/cvs/cv.de.yaml data/cvs/cv.en.yaml data/cvs/photos/markus.jpg
```
...
- [ ] **Step 32.3: Confirm no Markus content in baselines**
...
- **Before:** 162 commits, personal CV data (Markus Wiesecke) in HEAD + history
...
```bash
rm -rf /Users/markus/Developer/cvMake-OLD-2026-05-10
```
...
- [ ] Files `2026-05-10-personal-data-strategy-design.md` and `2026-05-10-personal-data-migration.md` are EXCEPTIONS in scrub (they reference Markus deliberately as context)
