...
└── photos/
    ...
    ├── alex.jpg           # gitignored (Maintainer's local file)
    └── alex.webp          # gitignored (Maintainer's local file)
```
...
- `apps/cli/test/build.integration.test.ts` (assertet `'Alex Schmidt'` + `'Berufserfahrung'`)
- `apps/web/components/sections/PersonalSection.test.tsx`
- Pattern: `s/Alex Schmidt/Lena Bauer/g`, `s/Schmidt/Bauer/g`
...
- `apps/web/app/dev-ui/page.tsx:63` — `placeholder="Alex"` → `placeholder="Alex"` (1 String)

### Group E — Docs/Specs/Plans (scrubben, behalten in History)

| File | Hits | Strategie |
|------|------|-----------|
| `docs/superpowers/plans/2026-04-24-cvmake-plan.md` | 54 | `s/Alex Schmidt/Alex Schmidt/g`, `s/Alex/Alex/g`, Email/Phone-Strings entfernen |
...
- `data/cvs/cv.de.yaml`, `cv.en.yaml`, `data/cvs/photos/alex.{jpg,webp}` — Real-Data
...
- `photos/alex.*` → out (catch-all `photos/*` minus `example-*`)
- `NEXT_SESSION.md` → out
- `.review-logs/` → out
...
**Subtilität:** `Alex` alleine (ohne Nachname) NICHT generell ersetzen — würde in Snapshots/Tests Kollisionen produzieren. Stattdessen: nach Operation A+B bleibt "Alex" nur noch in Files, die in Section-3-HEAD-Scrub bereits manuell editiert wurden (Group D + E). filter-repo rewritet die History dieser scrubbed Files mit ihrem neuen Content rückwirkend.
...
# .git/filter-repo/analysis/path-deleted-sizes.txt etc. — manuell auf 'alex' grepen
```
...
- PR #1 (Phase-8-Cleanup): 6 Commits, keiner berührt cv.*.yaml direkt. "Alex" nur in Test-Code-Diffs.
