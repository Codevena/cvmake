...
const ACTUAL_DIR = path.resolve('__tests__/__visual__/monochrome-dark/.actual');
...
describe('monochrome-dark visual baseline', () => {
  it('matched Baseline für mono-carbon', async () => {
    const png = await renderPageOneAsPng('mono-carbon');
    const baselinePath = path.join(BASELINE_DIR, 'mono-carbon.page1.png');
    await mkdir(ACTUAL_DIR, { recursive: true });
    await writeFile(path.join(ACTUAL_DIR, 'mono-carbon.page1.png'), png);
