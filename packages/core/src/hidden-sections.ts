 ...
 * always preserved. Idempotent: stripping already-stripped data is a no-op.
 *
 * This lives in core (not web) so the PDF export route AND the CLI build apply
 * it identically before rendering — otherwise a section hidden in the editor
 * preview would still print in the downloaded PDF (WYSIWYG divergence).
 *
 * NOTE: this module is intentionally free of Node-only imports (fs/sharp/etc.)
 * so it is safe to pull into a client bundle via the dedicated subpath export.
 */
